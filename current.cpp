...
void CURRENT::output_density(ofstream &ff)
{
	int i, N, j, k;
	double x;
	j = mygrid->Nloc[1] / 2;
	...
	for (i = 0; i < N; i++)
	{
		x = mygrid->rminloc[0] + i*mygrid->dr[0];
		ff << setprecision(6) << x << " " << density(i, j, k) << "\n";
	}
}

void CURRENT::output_2D_density(ofstream &ff)
{
	int i, Nx, Ny, j, k;
	double x, y;
	k = mygrid->Nloc[2] / 2;
	...
	for (j = 0; j < Ny; j++)
		for (i = 0; i < Nx; i++)
		{
			y = mygrid->rminloc[1] + j*mygrid->dr[1];
			x = mygrid->rminloc[0] + i*mygrid->dr[0];
			ff << setprecision(6) << x << " " << y << " " << density(i, j, k) << "\n";
		}
}
void CURRENT::output_2D_current(ofstream &ff)
{
	int i, Nx, Ny, j, k;
	double x, y;
	k = mygrid->Nloc[2] / 2;
	...
	for (j = 0; j < Ny; j++)
		for (i = 0; i < Nx; i++)
		{
			y = mygrid->rminloc[1] + j*mygrid->dr[1];
			x = mygrid->rminloc[0] + i*mygrid->dr[0];
			ff << setprecision(6) << x << " " << y << " " << Jx(i, j, k) << " " << Jy(i, j, k) << " " << Jz(i, j, k) << "\n";
		}
}
void CURRENT::output_2D_nth_comp(ofstream &ff, int c)
{
	int i, Nx, Ny, j, k;
	double x, y;
	k = mygrid->Nloc[2] / 2;
	...
	for (j = 0; j < Ny; j++)
		for (i = 0; i < Nx; i++)
		{
			y = mygrid->rminloc[1] + j*mygrid->dr[1];
			x = mygrid->rminloc[0] + i*mygrid->dr[0];
			ff << setprecision(6) << x << " " << y << " " << JJ(c, i, j, k) << "\n";
		}
}
void CURRENT::output_2D_nth_comp_human(ofstream &ff, int c)
{
	int i, Nx, Ny, j, k;
	k = mygrid->Nloc[2] / 2;
	Nx = mygrid->Nloc[0];
	...
	for (j = 0; j < Ny; j++){
		for (i = 0; i < Nx; i++)	{
			ff << setprecision(6) << JJ(c, i, j, k) << "\t";
		}
		ff << "\n\n";
	}
}
double CURRENT::integrate_field_nth_comp(int c)
{
	int i, Nx, Ny, j, k;
	double sum_integral;
	k = mygrid->Nloc[2] / 2;
...
void CURRENT::set_const_field_nthcomp(double value, int c)
{
	int i, j, k;
	int Nx, Ny, Nz;
